...
BEGIN
    ALTER TABLE dbo.users WITH CHECK ADD CONSTRAINT ck_users_rut_basic_format CHECK (
        rut IS NULL OR (
            LEN(rut) BETWEEN 9 AND 10
            AND LEN(rut) - LEN(REPLACE(rut, N'-', N'')) = 1
            AND CHARINDEX(N'-', rut) = LEN(rut) - 1
...
BEGIN
 CREATE TABLE dbo.reservation_target_audit(
  id BIGINT IDENTITY(1,1) PRIMARY KEY,reservation_id INT NOT NULL,actor_user_id INT NOT NULL,
  old_target_participants INT NULL,new_target_participants INT NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM dbo.reservations AS target
...
-- El trigger canónico ya valida PENDING y CONFIRMED. Se eliminan versiones
-- redundantes de instalaciones antiguas para preservar la semántica OPEN_USE.
IF OBJECT_ID(N'dbo.trg_reservations_pending_conflicts', N'TR') IS NOT NULL DROP TRIGGER dbo.trg_reservations_pending_conflicts;
IF OBJECT_ID(N'dbo.trg_blocks_pending_conflicts', N'TR') IS NOT NULL DROP TRIGGER dbo.trg_blocks_pending_conflicts;
IF OBJECT_ID(N'dbo.trg_scheduled_activities_pending_conflicts', N'TR') IS NOT NULL DROP TRIGGER dbo.trg_scheduled_activities_pending_conflicts;
GO
...
BEGIN
    ...
    IF TRY_CONVERT(bit, SESSION_CONTEXT(N'poliredi.skip_reservation_audit'))=1
        RETURN;
    ...
    SELECT
        ...
        CASE
            WHEN i.id IS NOT NULL AND d.id IS NULL THEN N'RESERVATION_CREATED'
            WHEN i.id IS NOT NULL AND d.id IS NOT NULL THEN N'RESERVATION_UPDATED'
            ELSE N'RESERVATION_DELETED'
        END,
        N'reservations',
        COALESCE(i.id,d.id),
        N'Cambio registrado sobre una reserva'
    FROM inserted AS i
    FULL OUTER JOIN deleted AS d ON d.id=i.id;
END;
GO
-- ============================================================
-- INDEXES DE SOPORTE OPERACIONAL
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.reservations') AND name=N'idx_reservations_resource_status_start')
    CREATE INDEX idx_reservations_resource_status_start ON dbo.reservations(resource_id,status,start_time) INCLUDE(duration_minutes,user_id,policy_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.reservations') AND name=N'idx_reservations_user_status_start')
    CREATE INDEX idx_reservations_user_status_start ON dbo.reservations(user_id,status,start_time) INCLUDE(duration_minutes,resource_id,policy_id,created_at);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.participants') AND name=N'idx_participants_user_status_reservation')
    CREATE INDEX idx_participants_user_status_reservation ON dbo.participants(user_id,status,reservation_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.availability_blocks') AND name=N'idx_availability_blocks_resource_active_start')
    CREATE INDEX idx_availability_blocks_resource_active_start ON dbo.availability_blocks(resource_id,is_active,start_time) INCLUDE(end_time);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.scheduled_activities') AND name=N'idx_scheduled_activities_resource_active_start')
    CREATE INDEX idx_scheduled_activities_resource_active_start ON dbo.scheduled_activities(resource_id,is_active,start_time) INCLUDE(end_time);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.workshop_enrollments') AND name=N'idx_workshop_enrollments_user_status')
    CREATE INDEX idx_workshop_enrollments_user_status ON dbo.workshop_enrollments(user_id,status,workshop_id);
IF EXISTS (
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.workshop_enrollments') AND name=N'uq_workshop_enrollments_confirmed')
