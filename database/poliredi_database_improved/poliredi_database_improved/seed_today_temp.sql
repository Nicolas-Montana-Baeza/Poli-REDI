...
IF TRY_CONVERT(bit, SESSION_CONTEXT(N'allow_today_seed')) <> 1
    THROW 59992, 'Seed bloqueado. Habilite allow_today_seed=1 en esta sesion.', 1;
GO
...
INSERT INTO dbo.reservations (
    ...
    duration_minutes, status, group_capacity_snapshot, target_participants,
    cancellation_reason
)
SELECT
    seed.id,
    ...
    'CONFIRMED',
    CASE WHEN group_scope.resource_id IS NOT NULL THEN resource.capacity END,
    CASE WHEN group_scope.resource_id IS NOT NULL THEN policy.minimum_participants END,
    NULL
FROM (VALUES
    (1, 2, 1, 1,  510, 60),
    (2, 3, 2, 2,  585, 90),
...
INNER JOIN dbo.reservation_policies AS policy ON policy.id=@initial_policy_id
INNER JOIN dbo.resources AS resource ON resource.id=seed.resource_id
LEFT JOIN dbo.reservation_policy_group_resources AS group_scope
  ON group_scope.policy_id=@initial_policy_id AND group_scope.resource_id=seed.resource_id;
