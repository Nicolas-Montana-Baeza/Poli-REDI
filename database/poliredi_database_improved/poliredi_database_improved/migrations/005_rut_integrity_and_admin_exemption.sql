...
-- Archivo: 005_rut_integrity_and_admin_exemption.sql
-- Propósito: Normalizar/proteger RUT y reinstalar validaciones canónicas de reservas.
-- Reejecución: Idempotente; valida datos antes de escribir.
...
BEGIN TRY
    ...
    SET rut = n.rut
    FROM dbo.users AS u
    INNER JOIN @normalized AS n ON n.user_id = u.id
    WHERE ISNULL(u.rut, N'') <> ISNULL(n.rut, N'');
    ...
    IF EXISTS (
        ...
        WHERE object_id = OBJECT_ID(N'dbo.users') AND name = N'ux_users_rut'
    )
    ...
    IF EXISTS (
        ...
        CROSS APPLY (
            SELECT CASE 11 - (calculation.weighted_sum % 11)
                WHEN 11 THEN N'0'
                WHEN 10 THEN N'K'
                ELSE CONVERT(NVARCHAR(1), 11 - (calculation.weighted_sum % 11))
            END
...
-- Los triggers canónicos anteriores ya cubren PENDING y CONFIRMED.
-- Se eliminan guards antiguos porque duplicaban reglas y bloqueaban OPEN_USE.
...
SELECT
    ...
    CASE WHEN OBJECT_ID('dbo.trg_users_rut_validate','TR') IS NOT NULL
         THEN 1 ELSE 0 END AS rut_validation_trigger_ok,
    CASE WHEN EXISTS (
    ...
    CASE WHEN OBJECT_ID('dbo.trg_reservations_pending_conflicts','TR') IS NULL
           AND OBJECT_ID('dbo.trg_blocks_pending_conflicts','TR') IS NULL
           AND OBJECT_ID('dbo.trg_scheduled_activities_pending_conflicts','TR') IS NULL
         THEN 1 ELSE 0 END AS redundant_pending_triggers_removed_ok;
GO
